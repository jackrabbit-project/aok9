...
Free, offline web app that runs an official **R.A.C.E. AOK9 Sprint Racing** meet per Rule Book v3.0:
entries → divisions → three programs of racing (draws, post positions, results) → final standings,
championship points and the official NRD report — and, if you want it, a live results page that
anyone at the meet can follow from a QR code.
...
**<https://aok9rms.gazehound.io>** — nothing to download, no account, and **free, always**. There is
no paid version, no subscription and nothing to unlock, and the same goes for every tool the
Jackrabbit Project publishes.
...
On the Setup screen, **Publish this meet** creates a page at `aok9rms.gazehound.io/r/<key>` that
...
What goes on the page is what the paddock board shows: call names, breeds, posts, results and
points. Owners, registration numbers, WAVEs, sex and notes are never sent. The page needs internet
to open; the app itself still works with none.
...
1. **Setup** — club, meet ID, date. Upload a newer official Grading Guide xlsx at any time
   ...
   **Results online** is here too: publish before printing anything and every sheet carries the
   QR code.
...
3. **Divisions** — *Auto-suggest* builds breed divisions (2+ same breed; mix types are their own
   ...
   divisions** makes the first sheet for the board.
...
6. **Results** — final standings per 4.3.5, trophies (5.1), BRC/MRC/National/TRC points
   ...
   are editable if the secretary needs to override a corner case. The results page's address is
   repeated here.
...
The rest of this file is for whoever runs the app from source, changes it or deploys it.
...
Warm paper, one accent colour, Fraunces for titles and IBM Plex Mono for labels and numbers. Both
fonts are self-hosted under the SIL Open Font License (see `LICENSE`) so they are in the offline
precache and the app looks the same with no signal. Everything is built for a phone at a field
first — big targets for entering results, tables that scroll inside their own box, numbers rounded
for reading. There is no dark theme; the tokens at the top of `src/styles.css` are laid out so one
can be added by redefining them.
...
### Serving the results page

The page is served by two Cloudflare Pages Functions in `functions/` (same project, same domain)
backed by a KV namespace. One-time setup in the Cloudflare dashboard:

1. **Workers & Pages → KV → Create a namespace**, e.g. `aok9-meets`.
2. **The Pages project → Settings → Bindings → Add → KV namespace**: variable name `MEETS`,
   pointing at that namespace. Add it for Production and for Preview.
3. Redeploy. Until the binding exists, publishing fails with a server error and the app shows
   *not published — retrying*; nothing else is affected.

Pages are kept for 400 days from the last update. What is sent is exactly `buildSnapshot()` in
`src/publish/snapshot.ts`, and `snapshot.test.ts` checks the published text for every field that
must stay out of it. The page lives on the same deployment as the app that made it, so a branch
preview publishes to its own preview address and the feature can be tried there before it is
merged. The functions do not run under `npm run dev`; to exercise them locally, build and run
`npx wrangler pages dev dist --kv MEETS`.
...
Two finished meets are bundled for **Try it out**. `sample-meet-1.json` is an old capture from the
real UI and is left alone; `sample-meet-2.json` is regenerated by the command above, which runs the
real domain engine over a roster of dogs taken from the bundled guide, so its draws, rotations and
championship points are the ones the app would actually produce. The roster is at the top of
`scripts/build-sample-meet.ts`, and `SAMPLE_SEED` rerolls the results. Editing either JSON by hand
is how you get a demo that disagrees with the rules it is demonstrating.

### Tests

`npm test` runs the unit tests. They cover Figure 8.1 (all rows), points tables 8.2A/8.2B,
dead-heat splitting, DQ redistribution, WAVE formulas, the rotation tie chain, and every worked
example in the rule book (TRC examples 5.7 #1–2, the eligible-entry example in 5.2), plus a
Grading Guide layout check, a report-export test, assertions run over both bundled example meets,
the header's step-progress rules and number formatting, and the results page — what the published
snapshot contains, what it must never contain, the publish endpoint's claim-and-update rules, and
the address the page is given.

## License

MIT — see [LICENSE](LICENSE). You may use, modify and redistribute this app freely; it comes with
no warranty of any kind. Using the app costs nothing and never will: no fee, no subscription, no
paid tier.
...
organize and manage AOK9 racing events. It is free to use, and always will be.
...
tools and sites built for the sighthound coursing and racing community. All of them are free,
always.
