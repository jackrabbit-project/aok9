...
in one line, with the button to turn publishing on for a meet that was scored without it. The page
lives on the same deployment as the app that made it, so a branch preview publishes to its own
preview address and the feature can be tried there before it is merged.
...
`snapshot.test.ts` checks the published text for each of them.
...
Pages are kept for 400 days from the last update. The functions do not run under `npm run dev`;
to exercise them locally, build and run `npx wrangler pages dev dist --kv MEETS`.
...
contains, what it must never contain, the publish endpoint's claim-and-update rules, and the address
the page is given.
...
uploaded unless you choose to publish a meet's results page (see below), and then only what the
paddock board shows. There is no account or cloud backup, which is also why the app works with no
internet. That means nobody can recover a lost meet for you: use *Save Meet to File* to keep your
own copies, and keep one from part-way through a long meet, not just at the end.
