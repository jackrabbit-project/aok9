...
import { PublishCtx, publishStatusText, usePublisher } from './publish/usePublisher';
...
function Shell() {
  ...
  // One publisher for the whole app, so there is one debounce and one queue.
  const publisher = usePublisher(state, dispatch);
  const published = publishStatusText(publisher.status, state.publish);
  const status = [
    ...
    .join(' · ');
  return (
    <PublishCtx.Provider value={publisher}>
      <div className="app">
